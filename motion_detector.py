...
def log_message(message, show=True, log_file=None):
    ...
    if show:
        print(log_message, flush=True)  # flush=Trueを追加
...
def save_video_with_audio_async(video_path, event, fps, width, height, cap_lock, output_queue):
    ...
    try:
        ...
        with subprocess.Popen(
            ...
            stdin=subprocess.DEVNULL,  # 標準入力を明示的に無効化
            start_new_session=True  # 新しいセッショングループで実行
        ) as process:
            try:
                stdout, stderr = process.communicate(timeout=60)  # タイムアウト追加
                ...
                process.kill()
                output_queue.put(f"FFmpeg timeout: {out_filename}")
                os.rename(temp_file, out_path)
...
def detect_motion_optimized(video_path, detection_dir, video_dir, threshold=25, min_area=300,
    ...
    try:
        ...
        video_save_executor.shutdown(wait=True)  # wait=Trueを明示
...
def process_video_parallel(args):
    """並列処理用のラッパー関数（修正版）"""
    # 子プロセスで標準入力のみを無効化（標準出力は維持）
    import sys
    import os
    
    # 標準入力のみ無効化してキーボード問題を防ぐ
    ...
    # ★ video_dir を detection_root 配下に移動
    video_dir = os.path.join(params['detection_root'], os.path.basename(video_dir))
    ...
    try:
        # デバッグ情報の表示
        if params['debug']:
            print(f"\n開始: {os.path.basename(video_path)}", flush=True)
...
def process_all_videos_parallel(input_dir, base_detection_dir="temporary_detection",
    ...
    # ★ output_video_xxx をまとめる親フォルダ: {input_dir}-detection/
    detection_root = f"{input_dir}-detection"
    os.makedirs(detection_root, exist_ok=True)
    print(f"出力先ルート: {detection_root}/")
    ...
    params = {
        ...
        'detection_root': detection_root,   # ★ 追加
        'threshold': threshold,
        'min_area': min_area,
        ...
        try:
            for result in executor.map(process_video_parallel, tasks):
                ...
                if result['status'] == 'skipped':
                    print(f"\n✓ スキップ: '{video_name}' - 既に処理済み", flush=True)
                    results['skipped'] += 1
                elif result['status'] == 'success':
                    print(f"\n✓ 処理完了: '{video_name}'", flush=True)
                    results['success'] += 1
                elif result['status'] == 'no_motion':
                    print(f"\n⚠ 動きが検出されませんでした: '{video_name}'", flush=True)
                    results['no_motion'] += 1
                else:
                    print(f"\n✗ 処理エラー: '{video_name}' - {result.get('error', 'Unknown error')}", flush=True)
                    results['error'] += 1
