...
- **購読者への通知はショート版だけ**（`NOTIFY_SUBSCRIBERS`）。まるごと版は毎日上がる長尺なので通知しない。
  `notifySubscribers` はアップロード時にしか効かず**後から変更できない**ので、上げ直さない限り取り消せない。
  どちらも通知したい／したくない日は `--notify all` / `--notify none`。
